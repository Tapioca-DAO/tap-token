...
function TOLPToken(props: { id: BigNumber }) {
    ...
    const handleMint = () => {
        tknMint
            ?.freeMint(BigNumber.from(mintAmount).mul((1e18).toString()))
            .then(async (tx) => {
                await tx.wait();
                tknData.refetch();
            });
    ...
    return (
        <Grid container direction="column" justifyContent="center">
            ...
            <Grid item margin="5px 0px 10px 0px">
                <Typography>
                    Balance: {formatBigNumber(tknData.data)}
                </Typography>
            </Grid>
...
function TOLPYBDeposit(props: { id: BigNumber }) {
    ...
    const allowance = useErc20MockAllowance({
        address: tknAddress,
        args: [address ?? '', ADDRESSES.yieldBox],
    });

    const yb = useYieldBox({
        address: ADDRESSES.yieldBox as any,
        signerOrProvider: signer,
    });
    const { data: shareBalanceOf, refetch: refetchBalance } =
        useYieldBoxBalanceOf({
            address: ADDRESSES.yieldBox as any,
            args: [address ?? '', props.id],
        });
    const balanceOf = useYieldBoxToAmount({
        address: ADDRESSES.yieldBox as any,
        args: [props.id, shareBalanceOf, false],
    });
    ...
    const onDeposit = async () => {
        await (
            await yb?.depositAsset(
                props.id,
                address,
                address,
                BigNumber.from(depositAmount).mul((1e18).toString()),
                0,
            )
        )?.wait();
    ...
    return (
        <Typography>
            <Button
                variant="text"
                onClick={onApprove}
                disabled={allowance.data?.gt(0)}
            >
                Approve
            </Button>
            <Button
                variant="text"
                disabled={Number(depositAmount) === 0 || allowance.data?.eq(0)}
                onClick={onDeposit}
            >
                Deposit
            </Button>
            <Button
                variant="text"
                disabled={balanceOf.data?.eq(0) || allowance.data?.eq(0)}
                onClick={onWithdraw}
            >
                Withdraw
            </Button>
            <TextField
                ...
                onChange={(e) => {
                    ...
                    if (value > tknData.data!.div((1e18).toString()).toNumber())
                        setDepositAmount(
                            tknData.data!.div((1e18).toString()).toString(),
                        );
            ...
            />
            {tknSymbol.data} to/from YieldBox. Balance:{' '}
            {formatBigNumber(balanceOf?.data ?? BigNumber.from(0))}
        </Typography>
    );
}
...
function TOLPLock(props: { id: BigNumber }) {
    ...
    const { data: shareBalanceOf } = useYieldBoxBalanceOf({
        address: ADDRESSES.yieldBox as any,
        args: [address ?? '', props.id],
    });
    const { data: balanceOf } = useYieldBoxToAmount({
        address: ADDRESSES.yieldBox as any,
        args: [props.id, shareBalanceOf, false],
    });
    ...
    const ybApproval = useYieldBoxIsApprovedForAll({
        address: ADDRESSES.yieldBox as any,
        args: [address ?? '', ADDRESSES.tOLP],
    });
    ...
    const onApprove = async () => {
        await (
            await yb?.setApprovalForAll(ADDRESSES.tOLP as any, true)
        )?.wait();
    ...
    return (
        <Typography>
            ...
            <Button
                ...
                onClick={onLock}
                disabled={
                    Number(depositAmount) === 0 || Number(durationAmount) === 0
                }
            >
                Lock
            </Button>
            <TextField
                ...
                onChange={(e) => {
                    ...
                    if (value > balanceOf!.div((1e18).toString()).toNumber())
                        setDepositAmount(
                            balanceOf!.div((1e18).toString()).toString(),
                        );
                    else if (value < 0) setDepositAmount('0');
                    else setDepositAmount(String(value ?? 0));
                }}
...
function TOLP() {
    const assetIds = useTapiocaOptionLiquidityProvisionGetSingularities({
        address: ADDRESSES.tOLP as any,
    });
