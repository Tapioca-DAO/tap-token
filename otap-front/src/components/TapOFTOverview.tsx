...
function TapOFTOverview() {
    ...
    const tapOftTotalSupply = useTapOftTotalSupply({
    ...
    const { data: tapPrice } = useOracleMockGet({
        address: ADDRESSES.tapOracle as any,
        args: ['0x00'],
    });
    ...
    const tapPriceMemo = useMemo(() => {
        if (tapPrice?._rate) {
            ...
            const decFrac =
                tapPrice?._rate.div((1e7).toString()).toNumber() - decNum * 10;
    ...
    return (
        <>
            ...
            <span>
                <Typography>
                    Total supply: {formatBigNumber(tapOftTotalSupply.data)}
                </Typography>
                <Typography>$TAP PRICE: {tapPriceMemo}</Typography>
                <Typography>
                    My tap balance: {formatBigNumber(tapOftBalance.data)}
                </Typography>
            </span>
        </>
    );
