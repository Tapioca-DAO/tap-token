...
def __init__(token_addr: address, _name: String[64], _symbol: String[32], _version: String[32]):
    ...
    @param token_addr TAP OFT address
    ...
    assert _decimals <= 255,"decimals not valid"
...
def changeController(_newController: address):
    ...
    """
    assert msg.sender == self.controller,"unauthorized"
...
def balanceOfAt(addr: address, _block: uint256) -> uint256:
    ...
    # reference yet
    assert _block <= block.number,"block not valid"
...
def create_lock(_value: uint256, _unlock_time: uint256):
    ...
    assert _value > 0,"value not valid"  # dev: need non-zero value
    assert _locked.amount == 0, "locked amount not valid"
    assert unlock_time > block.timestamp, "unlock time not valid"
    assert unlock_time <= block.timestamp + MAXTIME,"unlock time too big"
...
def increase_unlock_time(_unlock_time: uint256):
    ...
    assert _locked.end > block.timestamp, "locked end not valid"
    assert _locked.amount > 0, "locked amount not valid"
    assert unlock_time > _locked.end, "unlock time not valid"
    assert unlock_time <= block.timestamp + MAXTIME, "unlock time too big"
