...
token: public(address)  # TapOFT token
...
def add_gauge(addr: address, gauge_type: int128, weight: uint256):
    ...
    assert (gauge_type >= 0) and (gauge_type < self.n_gauge_types),"gauge type not valid"
    assert self.gauge_types_[addr] == 0,"gauge already exists"  # dev: cannot add the same gauge twice
...
def vote_for_gauge_weights(_gauge_addr: address, _user_weight: uint256):
    ...
    assert (_user_weight >= 0) and (_user_weight <= 10000),"user weight not valid"
    assert block.timestamp >= self.last_user_vote[msg.sender][_gauge_addr] + WEIGHT_VOTE_DELAY,"cannot vote at the moment"
    ...
    assert (power_used >= 0) and (power_used <= 10000),"power not valid"
