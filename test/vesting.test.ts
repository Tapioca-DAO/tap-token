...
describe.only('Vesting', () => {
    ...
    it('should test init', async () => {
        ...
        await expect(vesting.connect(eoa1).init(usdc.address, mintAmount)).to.be.revertedWith('Ownable: caller is not the owner');
        await expect(vesting.init(usdc.address, 0)).to.be.revertedWithCustomError(vesting, 'NoTokens');

        await vesting.init(usdc.address, mintAmount);
        await expect(vesting.init(usdc.address, mintAmount)).to.be.revertedWithCustomError(vesting, 'Initialized');
    ...
    it('should register users', async () => {
        ...
        await expect(vesting.registerUser(deployer.address, mintAmount.div(20))).to.be.revertedWithCustomError(
            vesting,
            'AlreadyRegistered',
        );

        await expect(vesting.init(usdc.address, mintAmount.div(100))).to.be.revertedWithCustomError(vesting, 'NotEnough');
        await vesting.init(usdc.address, mintAmount);

        await expect(vesting.registerUser(eoa1.address, mintAmount.div(5))).to.be.revertedWithCustomError(vesting, 'Initialized');
