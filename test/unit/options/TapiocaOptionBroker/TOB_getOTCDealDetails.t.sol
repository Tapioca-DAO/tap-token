...
contract TOB_getOTCDealDetails is TobBaseTest {
    function test_RevertWhen_OptionExpired() external setupAndParticipate(100) skipEpochs(2) {
        // it should revert
        vm.expectRevert(TapiocaOptionBroker.OptionExpired.selector);
        tob.getOTCDealDetails(1, ERC20(address(0x1)), 1);
    }

    function test_RevertWhen_PaymentTokenNotSupported() external setupAndParticipate(100) {
        // it should revert
        vm.expectRevert(TapiocaOptionBroker.PaymentTokenNotSupported.selector);
        tob.getOTCDealDetails(1, ERC20(address(0x1)), 1);
    }

    function test_RevertWhen_InEpochCooldown() external setDaiMockPaymentToken setupAndParticipate(100) {
        // it should revert
